      ...
      <div className="mb-8">
        ...
        <p className="mt-3 leading-6 text-stone-600">
          {forgotMode
            ...
            : signupMode
              ? "Create your own private baking notebook."
              : "Your baking notebook is private."}
        </p>
      </div>

      <form action={forgotMode ? requestPasswordReset : signupMode ? signUp : login} className="space-y-5 rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
        <div>
        ...
        <button type="submit" className="min-h-12 w-full rounded-xl bg-stone-900 px-5 py-3 font-semibold text-white">
          {forgotMode ? "Send reset link" : signupMode ? "Create account" : "Sign in"}
        </button>
      </form>

      <div className="mt-5 space-y-3 text-center text-sm text-stone-600">
        {forgotMode ? (
          <a href="/login" className="font-semibold text-stone-900 underline underline-offset-4">Back to sign in</a>
        ) : signupMode ? (
          <a href="/login" className="font-semibold text-stone-900 underline underline-offset-4">Already have an account? Sign in</a>
        ) : (
          <>
            <a href="/login?mode=forgot" className="block font-semibold text-stone-900 underline underline-offset-4">Forgot your password?</a>
            <a href="/login?mode=signup" className="block font-semibold text-stone-900 underline underline-offset-4">New here? Create an account</a>
          </>
        )}
      </div>
    </main>
