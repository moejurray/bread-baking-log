...
export default function ProcessForm({
  ...
  initialBaking: BakingStage[];
  initialCooling: ProcessStep[];
}) {
  ...
  const [openSection, setOpenSection] = useState<SectionName | null>(
    initialCooling.some((step) => step.duration_minutes !== null)
      ? "cooling"
      : initialBaking.some((stage) => stage.duration_minutes !== null)
        ? "baking"
        : "process"
  );
  ...
  const cooling = coolingNames.map(
    (name) =>
      initialCooling.find((step) => step.description === name) ?? {
        step_type: "resting",
        description: name,
  ...
  function addStep() {
    setSteps((current) => [
      ...current,
      { step_type: "other", description: "", duration_minutes: null, temperature_f: null },
    ]);
  }
  ...
  function deleteBakingStage(index: number) {
    setBaking((current) => current.filter((_, itemIndex) => itemIndex !== index));
  }
  ...
  function sectionHeader(title: string, subtitle: string, section: SectionName) {
    ...
    return (
      ...
      >
        ...
        <span className="text-xl text-stone-400">{isOpen ? "−" : "+"}</span>
      </button>
    );
  }
  ...
  return (
    <form ref={formRef} onChange={handleChange} className="space-y-4">
      <div className="sticky top-3 z-10 flex justify-end pointer-events-none">
        <div className={`rounded-full border bg-white/95 px-3 py-1.5 text-xs font-semibold shadow-sm ${saveState === "error" ? "border-red-200 text-red-700" : "border-stone-200 text-stone-600"}`}>
          {saveState === "saving"
            ? "Saving…"
            : saveState === "unsaved"
              ? "Changes pending…"
      ...
      <section className="rounded-3xl border border-stone-200 bg-white shadow-sm">
        {sectionHeader(
          "Process",
          openSection === "process" ? "Drag steps or use the arrows to put them in order." : `${steps.length} process steps`,
          "process"
        )}
            ...
            {steps.map((step, index) => (
              ...
              >
                <div className="mb-3 flex items-center justify-between">
                  <span className="cursor-grab text-sm font-semibold text-stone-500">☰ Step {index + 1}</span>
                  <div className="flex gap-2">
                    <button type="button" onClick={() => moveStep(index, index - 1)} disabled={index === 0} className="min-h-9 min-w-10 rounded-lg border bg-white disabled:opacity-30">↑</button>
                    <button type="button" onClick={() => moveStep(index, index + 1)} disabled={index === steps.length - 1} className="min-h-9 min-w-10 rounded-lg border bg-white disabled:opacity-30">↓</button>
                    <button type="button" onClick={() => deleteStep(index)} className="flex h-9 w-9 items-center justify-center rounded-full border bg-white">−</button>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <label className="text-sm font-medium">Step
                    <select name="step_type" defaultValue={step.step_type} className="mt-2 min-h-12 w-full rounded-xl border bg-white px-3">
                  ...
                  <label className="text-sm font-medium">Minutes
                    <input name="step_duration" type="number" min="0" defaultValue={step.duration_minutes ?? ""} onWheel={preventWheelChange} className="mt-2 min-h-12 w-full rounded-xl border px-3" />
                  </label>
                </div>
                <label className="mt-3 block text-sm font-medium">What did you do?
                  <input name="step_description" defaultValue={step.description ?? ""} className="mt-2 min-h-12 w-full rounded-xl border px-3" />
                </label>
                <label className="mt-3 block text-sm font-medium">Temperature °F <span className="font-normal text-stone-400">(when useful)</span>
                  <input name="step_temperature" type="number" step="0.1" defaultValue={step.temperature_f ?? ""} onWheel={preventWheelChange} className="mt-2 min-h-12 w-full rounded-xl border px-3" />
                </label>
              </div>
            ))}
          </div>
      ...
      <section className="rounded-3xl border border-stone-200 bg-white shadow-sm">
        {sectionHeader(
          "Baking",
          openSection === "baking" ? "Record each oven stage." : `${baking.length} baking stages`,
          "baking"
        )}
            ...
            {baking.map((stage, index) => (
              <div key={index} className="rounded-2xl bg-stone-50 p-4">
                <div className="flex justify-end">
                  <button type="button" onClick={() => deleteBakingStage(index)} className="flex h-9 w-9 items-center justify-center rounded-full border bg-white">−</button>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <label className="text-sm font-medium">Oven °F
                    <input name="bake_temperature" type="number" defaultValue={stage.temperature_f ?? ""} onWheel={preventWheelChange} className="mt-2 min-h-12 w-full rounded-xl border px-3" />
                  </label>
                  <label className="text-sm font-medium">Minutes
                    <input name="bake_duration" type="number" defaultValue={stage.duration_minutes ?? ""} onWheel={preventWheelChange} className="mt-2 min-h-12 w-full rounded-xl border px-3" />
                  </label>
                </div>
                <div className="mt-3 grid grid-cols-[9rem_1fr] gap-3">
                  <label className="text-sm font-medium">Lid
                    <select name="bake_lid" defaultValue={stage.lid_on ? "on" : "off"} className="mt-2 min-h-12 w-full rounded-xl border bg-white px-3">
                      <option value="on">On</option>
                      <option value="off">Off</option>
                    </select>
                  </label>
                  <label className="text-sm font-medium">Note
                    <input name="bake_description" defaultValue={stage.description ?? ""} className="mt-2 min-h-12 w-full rounded-xl border px-3" />
                  </label>
                </div>
              </div>
            ))}
      ...
      <section className="rounded-3xl border border-stone-200 bg-white shadow-sm">
        {sectionHeader(
          "Cooling",
          openSection === "cooling" ? "Record cooling after the loaf leaves the oven." : "Cooling details",
          "cooling"
        )}
            ...
            {cooling.map((step) => (
              <div key={step.description} className="rounded-2xl bg-stone-50 p-4">
                ...
                <div className="mt-3 grid grid-cols-2 gap-3">
                  <label className="text-sm font-medium">Minutes
                    <input name="cooling_duration" type="number" defaultValue={step.duration_minutes ?? ""} onWheel={preventWheelChange} className="mt-2 min-h-12 w-full rounded-xl border px-3" />
                  </label>
                  <label className="text-sm font-medium">Temperature °F
                    <input name="cooling_temperature" type="number" defaultValue={step.temperature_f ?? ""} onWheel={preventWheelChange} className="mt-2 min-h-12 w-full rounded-xl border px-3" />
                  </label>
                </div>
              </div>
            ))}
